...
def conv_cond_concat(x, y):
     ...
     return tf.concat([
        x, y*tf.ones([y_shapes[0], x_shapes[1], x_shapes[2], x_shapes[3], y_shapes[3]])], 4)

def encode_img(input_tensor, output_size):
    down_outputs = []
    output = tf.contrib.layers.conv3d(
        ...
        normalizer_params={'scale': True}) #mod
    print('output after 1st layer: {}'.format(output.get_shape()))
    ...
    print('output after 5th layer: {}'.format(output.get_shape()))
    output = tf.contrib.layers.conv3d(
        ...
        normalizer_params={'scale': True}) #mod
    print('output after last layer: {}'.format(output.get_shape()))
...
def gated_deconv(inputs, skip_input, out_ch, y, size, size_slice,  scope='gated_deconv/'):
    """
 ...
 #   print(type(skip_input.get_shape()[1]))
    prob_map = tf.contrib.layers.fully_connected(y, size*size*size_slice,
        scope = scope+'fully', activation_fn=tf.nn.sigmoid, normalizer_fn=None)
    print(prob_map.get_shape())

    prob_map = tf.reshape(prob_map, [-1, size_slice, size, size, 1]) #mod
    print('prob map shape: {}'.format(prob_map.get_shape()))
    print('skip inputs shape: {}'.format(skip_input.get_shape()))
    output = tf.multiply(skip_input, prob_map)
    print(output.get_shape(), inputs.get_shape())

    output = tf.concat([inputs, output], 4) # use concat or add??
    output = tf.contrib.layers.conv3d_transpose(
...
def generator(down_outputs, z_s, z_r, y, batch_size):
    ...
    output = tf.contrib.layers.fully_connected(z, 1024*4*4*1, scope='fully1',
        activation_fn=None, normalizer_fn=tf.contrib.layers.batch_norm,
        normalizer_params={'scale': True}) #mod
    print('shape: {}'.format(output.get_shape()))
    output = tf.reshape(output,[-1,1024,4,4,1]) #mod
    print('processed output: {}'.format(output.get_shape()))
    #output = tf.transpose(output, perm=[0, 2, 2, 3, 1]) #mod
    output = tf.transpose(output, perm=[0, 4, 3, 2, 1])
    print('processed output: {}'.format(output.get_shape()))
    output = prelu(output)
    print('shape down output: {}'.format(down_outputs[5].get_shape()))
    output = gated_deconv(output, down_outputs[5], 1024, y, 4, 1, scope='gated_deconv1/')
    print("========After the first gated layer==============",output.get_shape())
    print('shape down output: {}'.format(down_outputs[4].get_shape()))
    output = gated_deconv(output, down_outputs[4], 512, y, 8, 2, scope='gated_deconv2/')
    print("========After the 2nd gated layer==============",output.get_shape())
    print('shape down output: {}'.format(down_outputs[3].get_shape()))
    output = gated_deconv(output, down_outputs[3], 256, y, 16, 4, scope='gated_deconv3/')
    print(output.get_shape())
    output = gated_deconv(output, down_outputs[2], 128, y, 32, 8, scope='gated_deconv4/')
    print(output.get_shape())
    output = gated_deconv(output, down_outputs[1], 64, y, 64, 16, scope='gated_deconv5/')
    print(output.get_shape())


    prob_map = tf.contrib.layers.fully_connected(y, 128*128*32,
        scope ='last_fully', activation_fn=tf.nn.sigmoid, normalizer_fn=None)
    prob_map = tf.reshape(prob_map, [-1, 32, 128, 128, 1])
    print('prob map: {}'.format(prob_map.get_shape()))
    print('down output: {}'.format(down_outputs[0].get_shape()))
    skip_out = tf.multiply(down_outputs[0], prob_map)
    print('skip out shape: {}'.format(skip_out.get_shape()))
    print('output: {}'.format(output.get_shape()))
 #   skip_out = tf.multiply(down_outputs[0], skip_out)
    output = tf.concat([output, skip_out], 4) # use concat or add??
    print('output: {}'.format(output.get_shape()))
    output = tf.contrib.layers.conv3d_transpose(
        output, 1, deconv_size, scope='deconv6', stride=2, padding='SAME',
        activation_fn=tf.nn.sigmoid, normalizer_fn=None) #mod
...
def discriminator(input_X, y, batch_size, gan_noise=0.01):
    ...
    cond_b = tf.reshape(cond,[batch_size, 1, 1, 1, 10])
    print(cond_b.get_shape())
...
def get_ssim_loss(img1, img2, size=11, sigma=1.5):
    ...
    for index in range(img1.shape[-1].value):
        mu1 = tf.nn.conv3d(img1s[index], window, strides=[1,1,1,1], padding='VALID') #mod
        mu2 = tf.nn.conv3d(img2s[index], window, strides=[1,1,1,1], padding='VALID') #mod
        mu1_sq = mu1 * mu1
        mu2_sq = mu2 * mu2
        mu1_mu2 = mu1 * mu2
        sigma1_sq = tf.nn.conv3d(img1s[index]*img1s[index], window, strides=[1,1,1,1], padding='VALID') - mu1_sq #mod
        sigma2_sq = tf.nn.conv3d(img2s[index]*img2s[index], window, strides=[1,1,1,1], padding='VALID') - mu2_sq #mod
        sigma12 = tf.nn.conv3d(img1s[index]*img2s[index], window, strides=[1,1,1,1], padding='VALID') - mu1_mu2 #mod
...
def super_resolution(inputs , out_ch):
    conv1 = conv3d(inputs, 32, 5, 1, '/conv1', False) #mod
    ...
    conv2 = conv3d(residual5, 32, 3, 1, '/conv2', True) #mod
    print('conv2======================', conv2.get_shape())
    conv2 = conv2 + conv1
    conv3 = conv3d(conv2, 128, 3, 1, '/conv3', True) #mod
    print('conv3======================', conv3.get_shape())
    conv5 = conv3d(conv3, 32, 3, 1, '/conv5', False) #mod
    print('conv5======================', conv5.get_shape())
    conv6 = conv3d(conv5, out_ch, 3, 1, '/conv6', False) #mod
...
def residual_block(incoming, num_outputs, kernel_size, scope, data_format = 'NHWC'):
    conv1 = tf.contrib.layers.conv3d(
        ...
        data_format=data_format, activation_fn=None, biases_initializer=None) #mod
...
def conv3d(inputs, num_outputs, kernel_size, stride, scope, norm=True, ac_fn = prelu,
           d_format='NHWC'):
    outputs = tf.contrib.layers.conv3d(
        inputs, num_outputs, kernel_size, scope=scope, stride=stride,
        data_format=d_format, activation_fn=None, biases_initializer=None) #mod
    if norm:
